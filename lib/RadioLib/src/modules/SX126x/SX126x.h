...
class SX126x: public PhysicalLayer {
  public:
    ...
    */
    ...
    /*!
      ...
      \returns \ref status_codes
    */
    int16_t switchLoRa(float bw, uint8_t sf, uint8_t cr, uint8_t syncWord, uint16_t preambleLength, float tcxoVoltage, bool useRegulatorLDO = false);
    ...
    */
    ...
    /*!
      ...
      \returns \ref status_codes
    */
    int16_t switchFSK(float br, float freqDev, float rxBw, uint16_t preambleLength, float tcxoVoltage, bool useRegulatorLDO = false);
  ...
  */
  ...
  int16_t setPacketParaFSK(uint16_t preambleLength, uint8_t crcType, uint8_t syncWordLength, uint8_t addrComp, uint8_t whitening, uint8_t packetType = SX126X_GFSK_PACKET_VARIABLE, uint8_t payloadLength = 0xFF, uint8_t preambleDetectorLength = SX126X_GFSK_PREAMBLE_DETECT_16);
  ...
  private:
    ...
    int16_t config(uint8_t modem, bool calib = true);
