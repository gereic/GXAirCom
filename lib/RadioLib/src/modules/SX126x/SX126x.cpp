...
int16_t SX126x::switchLoRa(float bw, uint8_t sf, uint8_t cr, uint8_t syncWord, uint16_t preambleLength, float tcxoVoltage, bool useRegulatorLDO) {
  // set mode to standby
  //int16_t state = standby();
  int16_t state = standby(SX126X_STANDBY_XOSC);
  RADIOLIB_ASSERT(state);

  // configure settings not accessible by API
  state = config(SX126X_PACKET_TYPE_LORA,false);
  RADIOLIB_ASSERT(state);
...
int16_t SX126x::switchFSK(float br, float freqDev, float rxBw, uint16_t preambleLength, float tcxoVoltage, bool useRegulatorLDO){
  // set mode to standby
  //int16_t state = standby();
  int16_t state = standby(SX126X_STANDBY_XOSC);
  RADIOLIB_ASSERT(state);
  ...
  // configure settings not accessible by API
  state = config(SX126X_PACKET_TYPE_GFSK,false);
  RADIOLIB_ASSERT(state);
...
int16_t SX126x::setPacketParaFSK(uint16_t preambleLength, uint8_t crcType, uint8_t syncWordLength, uint8_t addrComp, uint8_t whitening, uint8_t packetType, uint8_t payloadLength, uint8_t preambleDetectorLength){
  _preambleLengthFSK = preambleLength << 3; //length in Bits
  _crcTypeFSK = crcType;
  _syncWordLength = syncWordLength << 3; //length in Bits
  _addrComp = addrComp;
  _whitening = whitening;
  _packetType = packetType;
  return setPacketParamsFSK(_preambleLengthFSK, _crcTypeFSK, _syncWordLength, _addrComp, _whitening, _packetType, payloadLength, preambleDetectorLength);  
}
...
int16_t SX126x::config(uint8_t modem, bool calib) {
